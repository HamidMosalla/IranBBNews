<%@ Page Title="" Language="C#" MasterPageFile="~/Main.master" AutoEventWireup="true" CodeFile="BlogPost.aspx.cs" Inherits="BlogPost" %>
...
<div id="contentpost">
    ...
    <asp:LinkButton ID="NewsTitleLabel" Text='<%# Eval("BlogPostTitle") %>' CommandArgument='<%# Eval("BlogPostID") %>' onclick="Link_Click"   oncommand="Link_Command" runat="server"></asp:LinkButton>
            <br />
            <asp:Image ID="NewsImage" Width="70" Height="70" ImageUrl='<%# Eval("[BlogPostPicture]") %>' runat="server" />
            <br />
            <asp:Label ID="NewsDateLabel" runat="server" Text='<%# Eval("BlogPostDate") %>' ForeColor="Silver" Font-Size="Smaller" />
            <br />
            <asp:Label ID="NewsSummaryLabel" runat="server" Text='<%# Eval("BlogPostSummary") %>' />
                <br />
                <br />
            </span>
        </ItemTemplate>
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" 
        ...
        SelectCommand="SELECT [BlogPostID], [UserIDfk], [BlogPostTitle], [BlogPostDate], [BlogPostSummary], [BlogPostPicture], [BlogPostVisible], [SubjectIDfk] FROM [BlogPost] WHERE ([BlogPostVisible] = @BlogPostVisible) ORDER BY [BlogPostDate] DESC">
        <SelectParameters>
